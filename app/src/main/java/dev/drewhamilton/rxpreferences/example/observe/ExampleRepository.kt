...
open class ExampleRepository @Inject constructor(protected val preferences: RxPreferences) {
    ...
    fun getExampleStringOnce() = preferences.getStringOnce(
        Keys.EXAMPLE_STRING,
        Defaults.STRING
    )
        .subscribeOn(scheduler)

    fun getExampleStringStream() = preferences.getStringStream(
        Keys.EXAMPLE_STRING,
        Defaults.STRING
    )
        .subscribeOn(scheduler)

    fun getExampleIntOnce() = preferences.getIntOnce(
        Keys.EXAMPLE_INT,
        Defaults.INT
    )
        .subscribeOn(scheduler)

    fun getExampleIntStream() = preferences.getIntStream(
        Keys.EXAMPLE_INT,
        Defaults.INT
    )
        .subscribeOn(scheduler)
