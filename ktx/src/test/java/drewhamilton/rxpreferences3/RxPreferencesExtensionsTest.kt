...
class RxPreferencesExtensionsTest {
    ...
    private fun mockGet(type: PreferenceType, key: String, returnedValue: Any) {
        when (type) {
            ...
            PreferenceType.INT -> whenever(mockSharedPreferences.getInt(eq(key), any()))
                .thenReturn(returnedValue as Int)
            PreferenceType.LONG -> whenever(mockSharedPreferences.getLong(eq(key), any()))
                .thenReturn(returnedValue as Long)
